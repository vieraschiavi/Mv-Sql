...
name: ecc
description: >
  Layer ECC (github.com/affaan-m/ECC v2.2.0) instalado en MV SQL: reglas de estilo, testing,
  ...
  cargar el estándar que aplica al stack (typescript web).
---
...
| `.claude/rules/ecc/{typescript web}/` | Reglas del stack de este repo (pisan a las de `common` cuando chocan) |
...
1. Correr el gate real del repo y **pegar la salida cruda**, no un resumen:
   ```bash
   npm test   # suite completa: web + app-python
   npm run lint
   ```
   La suite cubre las dos mitades del producto (web y app-python) desde el mismo comando.
...
./scripts/ecc-instalar.sh typescript web  # forzando los stacks de este repo
```
